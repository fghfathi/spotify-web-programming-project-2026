...
class Song(models.Model):
    ...
    def listeners_count(self) -> int:
        """Number of *unique* users who have played this track.

        Repeated plays by the same user count once. Only authenticated users
        generate StreamEvents (the play endpoint requires auth), so every
        stream has a real user and "unique listener" == distinct user id.
        """
        related = getattr(self, "stream_events", None)
        if related is None:
            return 0
        return related.values("user").distinct().count()
