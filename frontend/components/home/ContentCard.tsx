...
interface ContentCardProps {
  ...
  onPlay?: () => void;
  // Extra line below the subtitle. Used by the recommendations section to show
  // why a song was suggested.
  footnote?: string;
  // Optional genre shown as a small pill under the subtitle (songs only).
  genre?: string;
}
...
export default function ContentCard({
  ...
  return (
    <div className="group w-40 shrink-0 rounded-xl border border-zinc-800 bg-zinc-900/70 p-3 transition hover:border-zinc-600 hover:bg-zinc-800/80 sm:w-44">
      ...
      {genre && (
        <span className="mt-1.5 inline-block rounded-full border border-zinc-700 bg-zinc-800/60 px-2 py-0.5 text-[10px] font-medium text-zinc-400">
          {genre}
        </span>
      )}
      {footnote && (
        <p className="mt-1.5 line-clamp-2 text-[11px] leading-snug text-emerald-400/80">
          {footnote}
        </p>
      )}
    </div>
