...
// Limits mirror the backend validators (Step 4): audio .mp3/.wav/.m4a/.flac up
// to 50MB (FLAC is lossless and larger), cover .jpg/.jpeg/.png up to 2MB.
const MAX_AUDIO_SIZE_MB = 50;
const MAX_COVER_SIZE_MB = 2;
const ALLOWED_AUDIO_EXTENSIONS = ["mp3", "wav", "m4a", "flac"];
  ...
  const validate = (): boolean => {
    ...
    if (!audioFile) {
      ...
      if (!ALLOWED_AUDIO_EXTENSIONS.includes(extension)) {
        nextErrors.audio = "Audio must be MP3, WAV, M4A, or FLAC.";
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <ArtistFileDropzone
          label="Audio File"
          accept=".mp3,.wav,.m4a,.flac,audio/flac"
          hint="MP3, WAV, M4A, or FLAC — up to 50MB"
          file={audioFile}
